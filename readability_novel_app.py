...
def generate_validation_report(df, metadata):
    ...
    with PdfPages(buffer) as pdf:
        ...
        plt.text(0.1, 0.59, f"Mean Dyslexia-Friendly Score: {mean_score:.2f}", fontsize=11, transform=fig.transFigure)
        ...
        fig, ax = plt.subplots(figsize=(8.5, 11))
        ax.axis('off')
        ax.set_title("Appendix: Feature Descriptions and Readability Guidance", fontsize=16, weight='bold', pad=20)
        appendix_text = """Sentence Count: Total number of sentences in the chapter. Higher counts may suggest better segmentation of ideas.
Word Count: Total number of words. Neutral by itself; used alongside other metrics.
Avg Sentence Length: Words per sentence. Lower is generally more readable. (Lower Z = better)
Avg Word Length: Characters per word. Shorter words are easier to decode. (Lower Z = better)
Avg Syllables per Word: Lower syllable complexity improves fluency. (Lower Z = better)
Passive Sentences: Passive voice can obscure meaning. Fewer is better. (Lower Z = better)
Rare/Abstract Words: High use of uncommon or conceptual words adds difficulty. (Lower Z = better)
Sensory Words: Use of descriptive language that evokes senses increases engagement. (Higher Z = better)
Dyslexia-Friendly Score: Composite score derived from weighted features above. (Higher Z = better)

Z-score Coloring Key:
- Blue (Low Z-score): Below average
- Red (High Z-score): Above average

Interpretation:
- For features positively correlated with readability (e.g. Sensory Words, Score), red is good.
- For negatively correlated features (e.g. Sentence Length, Syllables), blue is better.
"""
...
st.markdown("Upload a `.txt` or `.docx` file formatted with metadata and chapter headers.")
...
if uploaded_file:
    ...
    if st.button("Run Readability Analysis and Generate Report"):
        ...
        st.download_button("📘 Download Full Validation Report (.pdf)", data=pdf_buffer,
